...
inline void image_display_2(short***in,int h,int w)
{
	short ***out=qx_allocs_3(h,w,3);
	for(int y=0;y<h;y++) for(int x=0;x<w;x++){out[y][x][0]=in[y][x][0]; out[y][x][1]=in[y][x][1]; out[y][x][2]=0;}
	imdebug("rgb *auto b=16 w=%d h=%d %p",w,h,out[0][0]);
	qx_frees_3(out); out=NULL;
}
inline void image_display_4(short***in,int h,int w,int nr_image=1)
{
	short ***out=qx_allocs_3(h,w,4);
	for(int i=0;i<nr_image;i++)
	{
		for(int y=0;y<h;y++) for(int x=0;x<w;x++) for(int c=0;c<4;c++) out[y][x][c]=in[y][x][c+i*4];
		imdebug("rgba *auto b=16 w=%d h=%d %p",w,h,out[0][0]);
	}
	qx_frees_3(out); out=NULL;
}
