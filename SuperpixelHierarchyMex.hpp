...
public:
	...
	void init(int h, int w, int connect = 4, int iterSwitch = 4)
	{
		...
		m_arcmax = computeEdge(h,w,connect); m_arcnum = 0; //arc == edge
	...
	void getMeanColor(unsigned char *img, unsigned char *color, int nr_superpixel)
	{
		cout << nr_superpixel << endl;
		cout << img[1] << endl;
		double**color_mean = qx_allocd(nr_superpixel, 3);
		unsigned char**color_mean_u = qx_allocu(nr_superpixel, 3);
		int*nr_pixel_per_label = new int[nr_superpixel];
		memset(nr_pixel_per_label, 0, sizeof(int)*nr_superpixel);
		memset(color_mean[0], 0, sizeof(double)*nr_superpixel * 3);
		unsigned char *ptrs = img;
		for (int i = 0; i<m_vexnum; ++i)
		{
			int id = m_label[i];
			nr_pixel_per_label[id]++;
			color_mean[id][0] += *ptrs++;
			color_mean[id][1] += *ptrs++;
			color_mean[id][2] += *ptrs++;
		}
		for (int i = 0; i<nr_superpixel; i++) for (int c = 0; c<3; c++) 
			color_mean_u[i][c] = qx_round(color_mean[i][c] / nr_pixel_per_label[i]);

		unsigned char *ptrd = color;
		for (int i = 0; i<m_vexnum; ++i)
		{
			int id = m_label[i];
			qx_memcpy_u3(ptrd, color_mean_u[id]);
			ptrd += 3;
		}
		qx_freed(color_mean); color_mean = NULL;
		qx_freeu(color_mean_u); color_mean_u = NULL;
		if (color_mean != NULL) delete[] color_mean; color_mean = NULL;
		image_display_3(color, m_h, m_w);
	}
