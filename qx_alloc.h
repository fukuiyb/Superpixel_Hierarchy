...
template<class T> inline T**qx_alloc2(int r,int c) throw(...)
{
	T *a,**p;
	int i;
...
template<class T> inline void qx_free2(T**p)
{
	if(p!=NULL)
	{
		delete[] p[0];
...
template<class T> inline T***qx_alloc3(int n,int r,int c) throw(...)
{
  	T *a,**p,***pp;
    int i,j,rc=r*c;
...
template<class T> inline void qx_free3(T***p)
{
	if(p!=NULL)
	{
		delete[] p[0][0];
